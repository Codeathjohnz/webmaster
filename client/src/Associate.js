...
const Associate = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3001/Assprof')
  ...
  const handleSelect = (profile) => {
    ...
    history.push({
      pathname: `/Associates/${profile.djoStudID}`, //ahehehe
  ...
  const handleDelete = (profileId) => {
    ...
    if (isConfirmed) {
      ...
      fetch(`http://localhost:3001/Assprof/${profileId}`, {
        method: 'DELETE',
      })
        .then((response) => {
          if (response.ok) {
  ...
  return (
    <div className="container mt-5">
       <h1 className="mb-4" style={{ Color: 'white', textAlign: 'center', textShadow: '2px 2px white' }}>List of Associate Programmer</h1>
      <div className="mb-3">
        <input
          type="text"
