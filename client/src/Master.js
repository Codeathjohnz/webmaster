...
const Master = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3001/prof')
  ...
  const handleSelect = (profile) => {
    ...
    history.push({
      pathname: `/Jit/${profile.djoStudID}`,
  ...
  const handleDelete = (profileId) => {
    ...
    if (isConfirmed) {
      ...
      fetch(`http://localhost:3001/prof/${profileId}`, {
        method: 'DELETE',
      })
        .then((response) => {
          if (response.ok) {
  ...
  return (
    <div className="container mt-5">
       <h1 className="mb-4" style={{ Color: 'white', textAlign: 'center', textShadow: '2px 2px white' }}>List of Master Programmer</h1>
      <div className="mb-3">
        <input
          type="text"
