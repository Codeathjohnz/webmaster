...
const Dether = (props) => {
  ...
  const handleUpdate = () => {
    ...
    fetch(`http://localhost:3001/profiles/${updatedProfile.djoStudID}`, {
      method: 'PUT',
      body: formData,
    })
      ...
      .then((data) => {
        if (data.success) {
          ...
          history.push('/Senior'); // Update the path as needed
        } else {
  ...
  return (
    <div className="container mt-5 bg-dark text-light p-4">
      <h1 className="text-center mb-4">Senior Profile</h1>
      {updatedProfile ? (
        <div>
